...
const ProductDetailPage = () => {
    ...
    const handleAddToFavorites = async () => {
        ...
        try {
            const res = await axiosClient.post('/api/users/add-wishlist', {
                productId: product._id
            });
            
            if (res.status === 'OK' || res.status === 200 || res.success) {
                messageApi.open({
                    type: 'success',
    ...
    return (
        <WrapperContainer>
            ...
            {/* 👇 2. HIỂN THỊ COMPONENT REVIEW Ở ĐÂY 👇 */}
            {/* Truyền hàm reload dữ liệu để cập nhật ngay khi comment xong */}
            <WrapperLayout style={{ marginTop: '20px' }}>
                <ProductReviews 
                    product={product} 
                    onReviewSubmit={() => fetchProductDetails(id)} 
                />
            </WrapperLayout>
            {/* 👆 -------------------------------------- 👆 */}
