...
import { updateUser } from '../../redux/slides/userSlice'; // <--- NHỚ IMPORT CÁI NÀY
...
const OrderPage = () => {
    ...
    // --- 1. SỬA ĐỔI: Dùng trực tiếp dữ liệu từ Redux Store ---
    const user = useSelector(state => state.user.currentUser);
    const cart = useSelector(state => state.cart);

    const [deliveryAddress, setDeliveryAddress] = useState('');

    // --- 2. QUAN TRỌNG: Tự động cập nhật User mới nhất khi vào trang ---
    useEffect(() => {
        const fetchLatestUserData = async () => {
            // Lấy ID và Token hiện tại
            const userId = user?.id || localStorage.getItem('userId');
            const token = localStorage.getItem('access_token');
            
            if (userId && token) {
                try {
                    // Gọi API lấy chi tiết user (Đảm bảo endpoint backend của bạn đúng)
                    // Nếu endpoint của bạn khác (vd: /api/user/profile), hãy sửa dòng dưới
                    const res = await axiosClient.get(`/api/users/get-details/${userId}`, {
                        headers: { token: `Bearer ${token}` }
                    });

                    if (res?.data) {
                        // Cập nhật lại Redux với dữ liệu mới nhất (có addresses)
                        dispatch(updateUser({ ...res.data, access_token: token }));
                    }
                } catch (error) {
                    console.error("Không tải được thông tin user mới nhất:", error);
                }
            }
        };

        // Luôn gọi hàm này khi mount để đảm bảo địa chỉ không bị cũ
        fetchLatestUserData();
    }, [dispatch, user?.id]);


    // --- 3. LOGIC HIỂN THỊ ĐỊA CHỈ (Đã chuẩn hóa) ---
    useEffect(() => {
        if (user) {
            let address = '';
            
            // Ưu tiên 1: Lấy từ mảng addresses (Schema mới - Mongoose)
            if (user.addresses && user.addresses.length > 0) {
                ...
                if (defaultAddr) {
                    ...
                    ].filter(part => part); // Lọc bỏ giá trị null/undefined/rỗng
                    
                    address = parts.join(', ');
                }
            } 
            // Ưu tiên 2: Fallback cho Schema cũ (User cũ chưa update)
            ...
            // Xử lý chuỗi rác nếu có
            if (address === '[object Object]') address = '';

            // Tạo chuỗi hiển thị đầy đủ: Tên + SĐT + Địa chỉ
            ...
            if(address && userName) {
                ...
                setDeliveryAddress(''); // Không có gì hết
            }
        }
    }, [user]); // Chạy lại mỗi khi Redux user thay đổi

    // --- CÁC HÀM XỬ LÝ GIỎ HÀNG (GIỮ NGUYÊN) ---
    const syncCartToDB = () => {
        // Dùng debounce nhẹ để tránh gọi API quá nhiều
        setTimeout(async () => {
            try {
                const token = localStorage.getItem('access_token');
                if (!token) return;
                // Lưu ý: Lấy cart từ Redux state hiện tại thay vì localStorage để chính xác hơn
                // Tuy nhiên giữ logic cũ của bạn nếu nó đang chạy ổn
    ...
    const handleCheckout = () => {
        if (selectedItems.length === 0) {
            ...
            // Chuyển sang trang thanh toán kèm dữ liệu
